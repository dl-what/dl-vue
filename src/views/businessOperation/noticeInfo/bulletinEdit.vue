    ...
    <panel :show="true" title="基本信息">
      ...
      >
        <el-form-item prop="noticeName" label="公告名称">
          <el-input v-model="baseForm.noticeName" placeholder="请输入公告名称" maxlength="30" show-word-limit></el-input>
        </el-form-item>
        <el-form-item prop="noticeType" label="公告类型">
          <el-select v-model="baseForm.noticeType" @change="noticeTypeChange" placeholder="请选择公告类型">
            <el-option
        ...
        <el-form-item prop="templateArr" label="公告模板" v-if="baseForm.noticeShow===2">
          <el-select
...
export default {
  ...
  created() {
    this.init();
    if (this.$storage.get("userInfo").insId !== 1) {
      this.managerialType = [
        { label: "省级", value: 2 },
        { label: "地市", value: 3 },
        { label: "大厅", value: 4 }
  ...
  methods: {
    async init() {
      this.noticeId = Number(this.$route.query.id);
      let res = await this.$api.NoticeDetail({ data: this.noticeId });
      console.log(res.data);
      if (res && res.code === 0) {
        if (res.data.templateList.length === 1) {
          if (res.data.templateList[0].contentForm === 1) {
            this.contentType = 1;
          } else {
            this.contentType = null;
            this.runningTime = res.data.templateList[0].runningTime;
          }
        } else if (res.data.templateList.length > 1) {
          this.contentType = null;
        ...
        this.noticeShowChange(res.data.noticeShow);
        this.queryNoticeMmanagerial(res.data.managerialType);
        for (let key in this.baseForm) {
          this.baseForm[key] = res.data[key];
          this.baseForm.time = [
            res.data.periodStartTime,
            res.data.periodEndTime
          ];
          if (res.data.noticeShow === 2) {
            this.baseForm.templateArr = res.data.templateList.map(item => {
              return item.templateId;
            });
          } else {
            this.baseForm.templateId = res.data.templateList[0].templateId;
          }
          if (res.data.noticeIns) {
            this.baseForm.noticeIns = res.data.noticeIns
              .slice(1, res.data.noticeIns.length - 1)
              .split(",")
              .map(Number);
          }
          if (res.data.noticeChannel) {
            this.baseForm.noticeChannel = res.data.noticeChannel
              .slice(1, res.data.noticeChannel.length - 1)
              .split(",");
          }
        }
        this.timeForm.timeData = res.data.noticeProgramList.map(item => {
          let HH = item.noticeStartTime.slice(0, 2);
          let mm = item.noticeStartTime.slice(3, 5);
          let ss = item.noticeStartTime.slice(6);
          return { timeItem: new Date(2020, 1, 1, HH, mm, ss) };
        });
        let param = { managerialType: this.baseForm.managerialType };
        switch (this.baseForm.managerialType) {
          case 1:
            param.insList = [1];
            break;
          case 2:
            param.insList = this.baseForm.noticeIns;
            break;
          case 3:
            param.insList = this.baseForm.noticeIns;
            break;
          case 4:
            param.channelList = this.baseForm.noticeChannel;
            break;
        }
    ...
    managerialChange(val) {
      this.baseForm.noticeIns = [];
      this.baseForm.noticeChannel = [];
      if (val === 1) {
        this.queryNoticeArrange();
      } else {
        this.queryNoticeMmanagerial(val);
      }
    },
    //获取机构或大厅数据
    async queryNoticeMmanagerial(data) {
      let res = await this.$api.queryNoticeMmanagerial({ data: data });
      if (res && res.code === 0) {
        if (data == 2 || data == 3) {
    ...
    submit() {
      this.$refs.baseForm.validate(valid => {
        if (valid) {
          ...
          console.log(data);
          (async data => {
            let res = await this.$api.NoticeEdit({
              message: "提交成功",
              data
            });
          })(data);
