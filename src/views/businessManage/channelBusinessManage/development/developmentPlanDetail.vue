      ...
      (async data => {
        ...
        if (res && res.code == 0) {
          ...
          if (res.data != null) {
            ...
            if (data.insLevel == 2) { //市级不显示省级的信息
              
              this.gameData =  this.gameData.filter(item => { 
                return  item.prop != 'provinceNewSellingHall' &&  
                        item.prop != 'provinceSellingMachine' && 
                        item.prop != 'provinceCooperationHall' && 
                        item.prop != 'provinceCooperationMachine' 
              })
            }
          }
        } else {
