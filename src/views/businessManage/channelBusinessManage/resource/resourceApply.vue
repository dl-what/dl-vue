  ...
  <div class="vlt-card">
    ...
    >
      <el-tabs v-model="activeName">
          ...
          <panel title="基本信息" :show="true" style="margin-bottom:15px">
            <div class="vlt-edit-single">
              <div class="vlt-edit-wrap">
                ...
                <el-form-item label="申请时间" prop="preReceivDate">
                  <el-date-picker
                    v-model="form.preReceivDate"
                    type="datetime"
                    placeholder="请选择申请日期时间"
                    :picker-options="pickerOptions"
                    :defaule-time="defaultTime"
                    @change="changeSelect"
                  ></el-date-picker>
                </el-form-item>
              </div>
        ...
        <el-tab-pane label="资源申请流程图" name="2" v-if="isValid">
...
export default {
  ...
  data() {
    return {
      defaultTime: this.$moment(Date.now()).format("HH:mm:ss"),
      disabled: false,
      isValid: false,
      ...
      pickerOptions: {
        ...
        selectableRange: `${this.$moment(Date.now()).format(
          "HH:mm:ss"
        )}-23:59:59`
      },
      rules: {
    ...
    "form.preReceivDate": {
      handler(newValue) {
        ...
        if (now - time > 0) {
          ...
          this.pickerOptions.selectableRange = `${this.$moment(
            Date.now()
  ...
  methods: {
    ...
    // 提交申请
    channelResApply(data) {
      const self = this;
      (async data => {
        let res = await self.$api.channelResApply({
          message: "提交成功",
          data
        });
        if (res && res.code == 0) {
    ...
    save() {
      ...
      data.warehouseGoodsInfoList = JSON.parse(JSON.stringify(self.tableData));
      data.warehouseGoodsInfoList.forEach(item => {
        item.unitPrice = self.$common.accMul(item.unitPrice, 100);
        item.num = item.num * 1;
        item.amount = self.$common.accMul(item.amount, 100);
