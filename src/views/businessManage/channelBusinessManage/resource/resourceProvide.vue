  ...
  <div class="vlt-card">
    ...
    >
      <el-tabs v-model="activeName">
        ...
        <el-tab-pane label="资源发放流程图" name="2" v-if="isValid">
...
export default {
  ...
  methods: {
    ...
    //入库仓库
    async provideWarehouseIdList() {
      let res = await this.$api.provideWarehouseIdList();
      if (res && res.code == 0) {
        console.log(res.data);
        this.warehouseData = res.data.map(item => {
          return { label: item.warName, value: item.id };
        });
      }
    },
    // 获取所有物品
    ...
    //发放
    channelResProvide(data) {
      const self = this;
      (async data => {
        let res = await self.$api.channelResProvide({
          message: "提交成功",
          data
        });
        if (res && res.code == 0) {
