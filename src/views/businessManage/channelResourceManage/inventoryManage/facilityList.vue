...
<div class="vlt-card">
  ...
  <el-table ref="print" :data="tableData" border style="width: 100%" class="table-header-style">
    ...
    <el-table-column label="操作" fixed="right" min-width="140px">
      <template slot-scope="scope">
        <el-button :id="'facilityList-detail' + scope.$index" @click="facilityDetail(scope.row.goodsId)" type="primary" v-has="'facilityList-detail'" size="mini">查看</el-button>
      </template>
    </el-table-column>
  </el-table>
...
return {
  ...
  controlOptions: [
    { name: "打印", type: "primary", icon: "printer", permission: "facilityList-print"},
    { name: "导出当前", type: "", icon: "s-promotion", permission: "facilityList-exportCurrent"},
    { name: "导出全部", type: "", icon: "s-promotion", permission: 'facilityList-exportAll'},
  ],
  tableList:[
    // {label:"序号",value:"",type:"index"},
    {label:"设施名称",value:"goodsName", width: '200px'},
  ...
  printData: {
    ...
    props: [
      ...
      {prop: 'goodsName', label:'设施名称', width: ""},
...
methods: {
  ...
  search(val) {
    this.params.page = 1;
    this.params.param = {...val,goodsType: 4};
  ...
  async getOut(isAll) {
    ...
    aLink.href = url;
    aLink.setAttribute("download", "设施库存数据.xls");
