...
<div class="vlt-card">
  ...
  <el-table ref="print" :data="tableData" border style="width: 100%" class="table-header-style">
    ...
    <el-table-column label="操作" fixed="right" min-width="140px">
      <template slot-scope="scope">
        <el-button :id="'mountingsList-detail' + scope.$index" @click="mountingsDetail(scope.row.goodsName,scope.row.deviceModelId)" type="primary" v-has="'mountingsList-detail'" size="mini">查看</el-button>
      </template>
    </el-table-column>
  </el-table>
...
return {
  ...
  controlOptions: [
    { name: "打印", type: "primary", icon: "printer", permission: 'mountingsList-print'},
    { name: "导出当前", type: "", icon: "s-promotion", permission: 'mountingsList-exportCurrent'},
    { name: "导出全部", type: "", icon: "s-promotion", permission: 'mountingsList-exportAll'},
  ],
  tableList:[
    // {label:"序号",value:"",type:"index"},
    {label:"配件名称",value:"goodsName", width: '200px'},
    {label:"配件型号",value:"deviceModel", width: '200px'},
...
methods: {
  ...
  search(val) {
    this.params.page = 1;
    this.params.param = {...val,goodsType: 2}
  ...
  async getOut(isAll) {
    ...
    aLink.href = url;
    aLink.setAttribute("download", "配件库存数据.xls");
  ...
  mountingsDetail(goodsName,deviceModelId){
    this.$router.push({
