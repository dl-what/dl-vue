<template>
  <div class="search-bar-comp">
    <el-form :inline="true" :model="form" :label-width="labelWidth">
      <span class="form-item-line">
        <el-form-item label="仓库名称">
          <el-input v-model="form.warName" placeholder="请输入仓库名称" class="device-item"></el-input>
        </el-form-item>
      </span>
      <span class="form-item-line">
        <el-form-item label="仓库类型">
        <el-select v-model="form.warType" placeholder="请选择仓库类型" class="device-item">
          <el-option
            v-for="(list, index) in warTypeOptions"
            :key="index"
            :label="list.label"
            :value="list.value">
          </el-option>
      ...
      <span class="form-item-line">
        ...
        <el-select v-model="form.goodsType" @change="selectGoodsType" placeholder="请选择物品类型" class="device-item">
          <el-option
      ...
      <span class="form-item-line" v-if="form.goodsType != ''">
        <el-form-item label="物品名称">
        <el-select v-model="form.goodsName" placeholder="请选择物品名称" class="device-item">
          <el-option
      ...
      <span class="form-item-line" v-if="form.goodsType == 1 || form.goodsType == 2">
        <el-form-item label="物品型号">
        <el-select v-model="form.deviceModel" @change="selectGoodsModel" placeholder="请选择物品型号" class="device-item">
          <el-option
    ...
    <p class="tips-item" v-if="visiableTotal.show" >
      <i class="el-alert__icon el-icon-info"></i>
      <span>
        共搜索到<em>{{visiableTotal.total}}</em>条数据
      </span>
    </p>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        warTypeOptions:[{label:'中彩仓库',value: 1},{label:'省中心仓库',value: 2},{label:'地市仓库',value: 3},{label:'销售大厅',value: 4}],
        optionsType: [{label:'设备',value: 1},{label:'配件',value:2},{label:'耗材',value: 3},{label:'设施',value:4}],
        optionsName: [],
        optionsModel: [],
        resourceDatas: [],
        options: [
    ...
    watch: {
      'total': {
        handler (newVal, oldVal) {
          // console.log(newVal);
          Object.assign(this.visiableTotal, newVal);
        },
        deep: true,
        immediate: true
      },
