<template>
  ...
  <div class="vlt-card">
    ...
    <div class="el_table">
      <el-table :data="tableData" border  ref="print">
        ...
        <el-table-column prop="goodsName" label="配件名称"></el-table-column>
        <el-table-column prop="deviceModel" label="配件型号"></el-table-column>
        <el-table-column prop="deviceCode" label="配件序列号" min-width="120"></el-table-column>
        <el-table-column prop="insName" label="所属机构"></el-table-column>
        <el-table-column prop="faultDesc" label="损耗原因" show-overflow-tooltip width="250"></el-table-column>
        <el-table-column prop="repairStaffName" label="申请人员" min-width="120"></el-table-column>
...
export default {
  ...
  data() {
    return {
      // 搜索组件配置
      searchOptions: [
        { type: "select", prop: "goodsId", value: "", title: "配件名称", placeholder: "请选择", options: [] },
        { type: "select", prop: "modelId", value: "", title: "配件型号", placeholder: "请选择", options: [] },
        { title: "申请日期", type: "datepicker-range", prop: "createTime", value: "", option: ["开始时间", "结束时间"] },
        { type: "cascader", prop: "insId", value: "", title: "所属机构", options: [], setProps: { label: "text", value: "id", children: "children", checkStrictly: true } }
      ],
      controlOptions: [
        { name: "打印", type: "primary", icon: "printer", permission: 'accessoryLoss-print' },
        { name: "导出", type: "", icon: "s-promotion", permission: 'accessoryLoss-export' },
        // { name: "导出全部", type: "", icon: "s-promotion", permission: 'accessoryLoss-exportAll'}
      ],
      tableData: [],
      total: 0,
      // pageSize: 10,
      ...
      printData: {
        ...
        props: [
          ...
          {prop: 'goodsName', label:'配件名称',width:''},
          {prop:'deviceModel', label:'配件型号',width:''},
          {prop:'deviceCode', label:'配件编码',width:''},
  ...
  methods: {
    ...
    async getList() {
      ...
      let res = await this.$api.getQueryAccessoriesLossReportList({
        data: {
          ...this.params,
          param: this.searchParams
        }
      });
      console.log("配件损耗报表", res);
      if (res && res.code == 0) {
    ...
    async exportExcel() {   //导出
      ...
      let result = await this.$api.exportAccessoriesLossReportList({
        data: {
      ...
      aLink.href = url;
      aLink.setAttribute("download", "配件损耗报表.xls");
