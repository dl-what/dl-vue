<template>
  <div class="vlt-card">
    <panel title="上市计划" :show="true">
      <base-info :infoList="gameListPlanVo"></base-info>
    </panel>
  ...
  data() {
    return {
      gameListPlanVo: [
        ...
        { title: "计划编号", value: "", prop: "gameListCode" },
        // { title: "上市版本", value: "", prop: "versionNumber" },
        { title: "计划简介", value: "", prop: "gameSaleDesc" },
        { title: "计划状态", value: "", prop: "gameListStatus" },
        { title: "上市时间", value: "", prop: "gameListTime" },
        { title: "销售区域", value: "", prop: "gameSaleIns" },
    ...
    getMarketPlanDetail(data) {
      ...
      (async data => {
        ...
        if (res && res.code == 0) {
          ...
          // 计划信息
          if(res.data.gameListPlanVo) {
            let options = {
              1: '未开始',
              2: '已上市',
              3: '已结束',
            };
            self.gameListPlanVo.forEach((item)=>{
              item.value = res.data.gameListPlanVo[item.prop]
              if(item.prop == 'gameSaleIns') {
                item.value = self.getGameSaleIns(item.value)
              }
              if(item.prop == 'gameListStatus') {
                item.value = options[res.data.gameListPlanVo.gameListStatus]
              }
              if(item.prop == 'createTime') {
                item.value = this.$moment(res.data.gameListPlanVo.createTime).format('YYYY-MM-DD HH:mm:ss')
              }
              if (item.prop == 'gameListTime') {
                item.value = this.$moment(res.data.gameListPlanVo.gameListTime).format('YYYY-MM-DD HH:mm:ss')
              }
              if(item.prop == 'gameSaleChannel') {
                if(res.data.gameListPlanVo.gameSaleChannel=='all') {
                  item.value = '区域内全部大厅';
                }else{
                  let array=[];
                  res.data.gameListPlanVo.gameSaleChannel.split(';').forEach((item)=>{
                    if(item) {
                      array.push(item)  
                    }
                  })
          ...
          // 上市附件
          let marketList = res.data.gameListPlanVo
          if(marketList.filePath) {
            let array = marketList.filePath.split(',');
            array.forEach((item)=>{
              let obj = {};
              obj.name = item.split('/')[item.split('/').length-1];
              obj.url = item;
              obj.createTime = marketList.createTime&&this.$moment(marketList.createTime).format('YYYY-MM-DD HH:mm:ss');
              obj.createByName = marketList.userName||'';
