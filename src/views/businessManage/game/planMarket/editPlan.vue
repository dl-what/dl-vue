  ...
  <div class="vlt-card" ref="main">
    ...
    <div v-if="active==0" class="wrap">
      ...
      <el-row class="vlt-edit-btn">
        ...
        <el-button type="primary" size="medium" @click="next('0')" :id="$route.name+'-'+'planInfoNext'">下一步</el-button>
      </el-row>
    </div>
    <!-- 上传游戏包 -->
    <div v-if="active==1" class="wrap">
      ...
      <el-row class="vlt-edit-btn">
        <el-button size="medium" @click="prev" class="cancel" :id="$route.name+'-'+'uploadGamePrev'">上一步</el-button>
        <el-button type="primary" size="medium" @click="next('1')" :id="$route.name+'-'+'uploadGameNext'">下一步</el-button>
      </el-row>
    </div>
    <!-- 游戏配置 -->
    <div v-if="active==2" class="wrap">
      ...
      <el-row class="vlt-edit-btn">
        <el-button size="medium" @click="prev" class="cancel" :id="$route.name+'-'+'gameprev'">上一步</el-button>
        <el-button type="primary" size="medium" @click="next('2')" :id="$route.name+'-'+'gameNext'">下一步</el-button>
      </el-row>
    </div>
    <!-- 上传附件 -->
    <div class="wrap" v-if="active==3">
      ...
      <el-row class="vlt-edit-btn">
        <el-button size="medium" @click="prev" class="cancel" :id="$route.name+'-'+'uploadPrev'">上一步</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        gameListName: null, // 上市计划名称
        gameListCode: null, // 上市计划编号
        gameListTime: null, // 上市时间
        gameSaleIns: null, // 上市区域
        gameSaleChannel: '1', // 上市渠道
        gameSaleDesc: null, // 上市简介
        filePath: null,
        gameSaleChannelCode: '', // 渠道编号
        isOverride: 1, // 是否覆盖区域 0-不覆盖 1-覆盖
      },
      data: {
        gameInfoVo: { // 游戏信息 （参数）
          id: null,
  ...
  watch: {
    active(val) {
      if(val==0) {
        this.$refs.planInfo.isManual = true;
      }else{
        this.$refs.planInfo.isManual = false;
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    getMarketPlanDetail(data) {
      ...
      (async (data)=>{
        let res = await self.$api.getMarketPlanDetail({data})
          ...
          self.form = Object.assign(self.form, res.data.gameListPlanVo); // 计划详情参数


          // 销售区域回填
          // let area = res.data.gameListPlanVo.gameSaleIns.split(';');
          // let insArray = []
          ...
          //   }
          // })
          // self.form.gameSaleIns = insArray;
          // 大厅和终端数据处理
          if(self.form.gameSaleChannel=='all') {
            self.form.gameSaleChannel = '1';
          }else{
            let code = []
            let array = self.form.gameSaleChannel.split(';')
            array.forEach((item)=>{
              if(item) {
                code.push(item)
              }
            })
            self.form.gameSaleChannelCode = code.join(';');
            self.form.gameSaleChannel = '2';
          }
    ...
    next(flag) {
      ...
      if(flag == '0') {
        ...
        if(self.params.param.gameSaleChannel=='2'&&self.params.param.gameSaleChannelCode.trim()=='') {
        ...
        if(planInfo) {
          ...
          let param = {
            id: self.form.id,
            gameListCode: self.form.gameListCode,
            gameListName: self.form.gameListName,
          };
          self.checkListPlanIsExist(param)
        }else{
    ...
    // 校验上市计划是否存在
    checkListPlanIsExist(data) {
      const self = this;
      (async (data)=>{
				let res = await self.$api.checkListPlanIsExist({data})
				if(res && res.code == 0) {
          if(self.form.gameSaleChannel==1) {
            if (self.active++ > 3) self.active = 0;
            self.params.param.gameId = self.form.gameId;
            self.getGameConList(self.params)
          } else {
            let array = [];
            self.form.gameSaleIns.forEach(item=>{
              array.push(item[item.length-1])
            })
            let insIdStr = `;${array.join(';')};`;
            let channelNoStr = `;${self.form.gameSaleChannelCode};`;
            let param = {
              insIdStr,
              channelNoStr,
            };
    ...
    submit() {
      ...
      if(data.gameSaleChannel=='1') {
        data.gameSaleChannel = 'all'
      }else{
        let str = data.gameSaleChannelCode.replace(/；/g, ';')
        ...
        data.gameSaleChannel = `;${channelCode.join(';')};`;
      }
      delete data.gameSaleChannelCode;
      data.gameListTime = new Date(data.gameListTime).getTime();
      let array = [];
      data.gameSaleIns.forEach(item=>{
        array.push(item[item.length-1])
      })
      data.gameSaleIns = `;${array.join(';')};`
      console.log('提交的参数', data)
      self.editMarketPlan(data)
    },
    // 编辑上市计划
    editMarketPlan(data) {
      const self = this;
