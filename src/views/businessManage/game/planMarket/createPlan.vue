  ...
  <div class="vlt-card" ref="main">
      ...
      <panel title="计划信息" :show="true" :isBack="false" style="margin-bottom:15px">
        <div class="vlt-edit-single">
          <div class="vlt-edit-wrap plan-wrap">
            <plan-info v-model="form" ref="planInfo"></plan-info>
          </div>
        </div>
      </panel>
      <el-row class="vlt-edit-btn">
        <el-button size="medium" @click="back" class="cancel" :id="$route.name+'-'+'back'">返 回</el-button>
        <el-button type="primary" size="medium" @click="next('0')" :id="$route.name+'-'+'planInfoPrev'">下一步</el-button>
      </el-row>
    </div>
    <!-- 上传游戏包 -->
    <div v-if="active==1" class="wrap">
      ...
      <el-row class="vlt-edit-btn">
        ...
        <el-button type="primary" size="medium" @click="next('1')"  :id="$route.name+'-'+'UploadNext'">下一步</el-button>
      </el-row>
    </div>
    <!-- 游戏配置 -->
    <div v-if="active==2" class="wrap">
      ...
      <el-row class="vlt-edit-btn">
        <el-button size="medium" @click="prev" class="cancel"   :id="$route.name+'-'+'gameInfoPrev'">上一步</el-button>
        <el-button type="primary" size="medium" @click="next('2')"   :id="$route.name+'-'+'gameInfoNext'">下一步</el-button>
      </el-row>
    </div>
    <!-- 上传附件 -->
    <div class="wrap" v-if="active==3">
      ...
      <el-row class="vlt-edit-btn">
        <el-button size="medium" @click="prev" class="cancel"  :id="$route.name+'-'+'uploadAttachmentPrev'">上一步</el-button>
  ...
  methods: {
    ...
    next(flag) {
      ...
      if(flag == '0') {
        ...
        if(self.form.gameSaleChannel=='2'&&self.form.gameSaleChannelCode.trim()=='') {
      ...
      (async data => {
        ...
        if (res && res.code == 0) {
          if(res.data.isExist) {
            ...
            self.$message.error(`区域指定大厅编码为 ${text} 已存在，请重新输入`)
          }
        }
      })(data);
    },
    ...
    submit() {
      ...
      data.createBy = this.$storage.get('userInfo').userId;
