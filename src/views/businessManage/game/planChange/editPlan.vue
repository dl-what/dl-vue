          ...
          <div class="vlt-edit-wrap">
            <el-form label-position="right" 
              ...
              <el-form-item label="配置编码" prop="deployCode">
                <el-input v-model.trim="data.gameRuleVo.deployCode" disabled placeholder="请输入配置编码"></el-input>
              </el-form-item>
            </el-form>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        changePlanName: null, // 变更计划名称
        changePlanCode: null, // 变更计划编号
        changePlanTime: null, // 变更时间
        changePlanIns: null, // 变更区域
        changePlanChannel: '1', // 变更渠道
        changePlanDesc: null, // 变更简介
        filePath: null,
        changePlanChannelCode: '', // 变更渠道编号
      },
      data: {
        gameInfoVo: { // 游戏信息 （参数）
          id: null,
  ...
  methods: {
    ...
    getChangePlanDetail(data) {
      ...
      (async (data)=>{
        let res = await self.$api.getChangePlanDetail({data})
          ...
          self.form = Object.assign(self.form, res.data.gameChangePlanVo); // 计划详情参数


          // 销售区域回填
          // let area = res.data.gameChangePlanVo.changePlanIns.split(';');
          // let insArray = []
          ...
          //   }
          // })
          // self.form.changePlanIns = insArray;
          // 大厅和终端数据处理
          if(self.form.changePlanChannel=='all') {
            self.form.changePlanChannel = '1';
          }else{
            let code = []
            let array = self.form.changePlanChannel.split(';')
            array.forEach((item)=>{
              if(item) {
                code.push(item)
              }
            })
            self.form.changePlanChannelCode = code.join(';');
            self.form.changePlanChannel = '2';
          }
    ...
    getGameConDetail(data) {
      ...
      'returnRateInterval']
          ...
          if(data.gameRuleVo) {
            ...
            console.log(self.data.gameRuleVo)
          }
          // 资金规则
    ...
    next(flag) {
      ...
      if(flag == '0') {
        ...
        if(planInfo) {
          ...
          let param = {
            id: self.form.id,
            changePlanCode: self.form.changePlanCode,
            changePlanName: self.form.changePlanName,
          };
          self.checkChangePlanIsExist(param)
    ...
    // 校验变更计划是否存在
    checkChangePlanIsExist(data) {
      const self = this;
      (async (data)=>{
				let res = await self.$api.checkChangePlanIsExist({data})
				if(res && res.code == 0) {
          if (self.active++ > 3) self.active = 0;
          self.params.param.gameId = self.form.gameId;
          self.getGameConList(self.params)
    ...
    submit() {
      ...
      if(data.changePlanChannel=='1') {
        data.changePlanChannel = 'all'
      }else{
        let str = data.changePlanChannelCode.replace(/；/g, ';')
        data.changePlanChannel = `;${str};`;
      }
      delete data.changePlanChannelCode;
      data.changePlanTime = new Date(data.changePlanTime).getTime();
      let array = [];
      data.changePlanIns.forEach(item=>{
        array.push(item[item.length-1])
      })
      data.changePlanIns = `;${array.join(';')};`
      console.log('提交的参数', data)
      self.editChangePlan(data)
    },
    // 编辑变更计划
    editChangePlan(data) {
      const self = this;
