          ...
          <el-table-column fixed="right" label="操作" min-width="100">
            <template slot-scope="scope">
              <el-button
                ...
                :id="$route.name+'-'+'marketListDetail'+scope.$index"
              >查看</el-button>
            </template>
          </el-table-column>
        </el-table>
          ...
          <el-table-column fixed="right" label="操作" min-width="100">
            <template slot-scope="scope">
              <el-button
                ...
                :id="$route.name+'-'+'changeListDetail'+scope.$index"
              >查看</el-button>
            </template>
          </el-table-column>
        </el-table>
          ...
          <el-table-column fixed="right" label="操作" min-width="100px">
            <template slot-scope="scope">
              <el-button
                ...
                :id="$route.name+'-'+'leaveListDetail'+scope.$index"
              >查看</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    detail(name, param) {
      this.$router.push({name,query:{id: param.id, gameId: param.gameId, deployCode: param.deployCode, type: 1}})
    },
    // 下载
    async downLoad(val) {
        ...
        if (res && res.code == 0) {
          ...
          if(res.data.gameInfoVo) {
            ...
            self.gameInfoVo.forEach((item)=>{
              ...
              if(item.prop=='createTime') {
                item.value = res.data.gameInfoVo.createTime&&this.$moment(res.data.gameInfoVo.createTime).format('YYYY-MM-DD HH:mm:ss')
              }
              if(item.prop=='updateTime') {
                item.value = res.data.gameInfoVo.updateTime&&this.$moment(res.data.gameInfoVo.updateTime).format('YYYY-MM-DD HH:mm:ss')
              }
            })
          }
          // 开发商信息
          ...
          if(gameList.filePath) {
            ...
            array.forEach((item)=>{
              ...
              obj.url = item;
              obj.createTime = this.$moment(gameList.createTime).format('YYYY-MM-DD HH:mm:ss');
      ...
      (async data => {
        ...
        if (res && res.code == 0) {
          ...
          res.data.records.forEach(item => {
            ...
            item.startPlayTime = item.startPlayTime&&this.$moment(item.startPlayTime).format('YYYY-MM-DD HH:mm:ss')
            item.endPlayTime = item.endPlayTime&&this.$moment(item.endPlayTime).format('YYYY-MM-DD HH:mm:ss')
          })
      ...
      (async (data)=>{
        ...
        console.log(res);
          ...
          res.data.records.forEach(item => {
            ...
            item.createTime = this.$moment(item.createTime).format('YYYY-MM-DD HH:mm:ss')
            item.gameListTime = item.gameListTime&&this.$moment(item.gameListTime).format('YYYY-MM-DD HH:mm:ss')
          })
    ...
    eachList(arr, obj) {
      obj&&arr.forEach(item => {
        ...
        if (item.prop == 'gameType') {
          item.value = this.gameTypeOptions[obj[item.prop]]
        }
        if (item.prop == 'gameStatus') {
