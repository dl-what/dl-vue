  ...
  methods: {
    ...
    next(flag) {
      ...
      if(flag == '0') {
        ...
        if(self.form.gamePlayChannel=='2'&&self.form.gamePlayChannelCode.trim()=='') {
    ...
    submit() {
      ...
      if(data.gamePlayChannel=='1') {
        ...
        data.gamePlayChannel = `;${str};`;
      }
      delete data.gamePlayChannelCode;
      
      // 试玩时间
      data.startPlayTime = new Date(data.playTime[0]).getTime();
      data.endPlayTime = new Date(data.playTime[1]).getTime();
      ...
      self.createTryPlan(data)
    },
    // 创建试玩计划
    createTryPlan(data) {
      const self = this;
      (async (data)=>{
				let res = await self.$api.createTryPlan({data})
				if(res && res.code == 0) {
