          ...
          <div class="vlt-edit-single">
            <div class="vlt-edit-wrap">
              <el-form-item label="配置编码" prop="deployCode">
                <el-input v-model="data.gameRuleVo.deployCode" placeholder="请输入配置编码"></el-input>
              </el-form-item>
            </div>
          </div>
        </panel>
        ...
        <panel title="自定义设置(可选)" :show="true" :isBack="false" style="margin-bottom:15px">
          <div class="vlt-edit-single">
            <div class="vlt-edit-wrap">
              ...
              <el-form-item label="自定义设置" prop="deployCode">
                <el-input
...
export default {
  ...
  created() {
    this.getGameConDetail(this.params)
  },
  watch: {
    params: {
      handler(val) {
        this.getGameConDetail(val)
      },
      deep: true,
    }
  },
      ...
      (async (data)=>{
        let res = await self.$api.getChangePlanDetail({data})
          ...
          if(self.form.changePlanChannel=='all') {
            ...
            self.form.changePlanChannelCode = self.form.changePlanChannel;
