<template>
  <div class="vlt-card addSubject" >
    <div class="vlt-edit-single">
      <panel title="编辑课程" :show="true">
      <div class="vlt-edit-wrap">
        <el-form ref="ruleForm" :model="form" :rules="rules" label-position="right" label-width="90px">
          <el-form-item label="课程名称" prop="name">
            <el-input v-model="form.name" size="small" placeholder="请输入课程名称"></el-input>
          </el-form-item>
          <el-form-item label="课程类型" prop="courseTypeId">
            <el-select v-model="form.courseTypeId" filterable placeholder="请选择课程类型" >
              <el-option v-for="item in courseTypeList" :key="item.value" :label="item.label" :value="item.value" ></el-option> </el-select>
          </el-form-item>
          ...
          <el-form-item label="课程简介" prop="description">
            <el-input type="textarea" :rows="3" placeholder="请输入课程简介" v-model="form.description"  maxlength="150" show-word-limit> </el-input>
          </el-form-item>
          ...
          <div class="btn-import">
            <div class="btn-item">
                <el-tooltip  effect="light" content="提示：只能导入以.xls/.xlsx为后缀的文件格式！" placement="top" >
                  <el-upload style="display:flex;align-items:flex-end;" action :auto-upload="true"
                   :on-success="handleAvatarSuccess" :http-request="uploadFileImg"
          ...
          <div class="wrapper">
            <el-table :data="selectTableData" border style="width: 100%;">
              ...
              <el-table-column prop="questionType" label="题目类型" align="center" min-width="120">
                <template slot-scope="scope">{{scope.row.questionType===1?"多选题":"单选题"}}</template>
              </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      courseModeList: [
        ...
        {value: 3, label:'图片', format: '.jpg,.jpeg,.png,.gif,.bmp,.JPG,.JPEG,.PBG,.GIF,.BMP'},
      ...
      rules: {
        ...
        courseTypeId: [{ required: true, message: '课程类型不能为空', trigger: ["change","blur"] }],
  ...
  created(){
    ...
    if (this.id) {
      this.queryTrainCourseInfo(this.id);
    }
  ...
  methods: {
    ...
    // 查询课程需修改的数据 
    queryTrainCourseInfo (id) {
      const _this = this;
      const data = {
        id
      };
      (async data => {
        let res = await _this.$api.queryTrainCourseInfo({data});
        if (res && res.code === 0) {
          console.log(res);
          Object.assign(_this.form, res.data);
          _this.form.questionId = _this.form.questionId ? _this.form.questionId.split(',').map(item=> Number(item)) : [];

          if (_this.form.courseMode) {
            _this.changeFileType (Number(_this.form.courseMode));
          }
          if (_this.form.url) {
            let arrs = _this.form.url.split(',')

            _this.oType = _this.form.courseMode;
            _this.courseModeTypeList[Object.keys(_this.courseModeTypeList)[_this.oType  - 1]] = arrs;
            // for(let i = 0; i < arrs.length; i++) {
            //   console.log(arrs[i]);
            //   _this.fileList.push(arrs[i]);
            // }
            arrs&&arrs.forEach((item)=>{
              let obj = {};
          ...
          _this.getQueryTrainQuestionList(params);
      ...
      (async data => {
        ...
        if (res && res.code === 0) {
          ...
          res.data.records&&res.data.records.forEach(item => {
            ...
            // })
            if (_this.form.questionId.indexOf(item.id) > -1) {
              _this.selectTableData.push(item)
            }
          });
        }
      })(data)
    ...
    changeFileType (type) {
      ...
      console.log(this.courseModeTypeList, Object.keys(this.courseModeTypeList)[this.oType]);
    ...
    sendQuestionArr (arr) {
      ...
      console.log('传过来的值', this.form.questionId, arr);
    },
    ...
    submit(){
      ...
      _this.$refs.ruleForm.validate(valid => {
        ...
        if (valid) {
          ...
          data.courseTypeId = _this.form.courseTypeId;
          data.id =  this.id;
          delete data.courseTypeId;
          data.questionId = [...new Set([..._this.form.questionId])].join(',');
          console.log('request', data);

          (async (data) => {
            let res = await _this.$api.trainCourseUpdate({ message: '提交成功', data });
            if (res && res.code == 0) {
              console.log(res);
              _this.cancel();
            }
