...
export default {
  ...
  checkEmptyNumber: (rule, value, callback) => {
    ...
    if (_value && !test) {
      return callback(new Error(`${rule.title}必须输入数字`));
    }
    callback();
  },
  // 只能输入正整数
  ...
  unDonationAmount: (rule, value, callback) => {
    ...
    const reg = /^\d+(\.\d{0,2})?$/g;
    if (_value && !reg.test(_value)) {
      return callback(new Error(`${rule.title}必须为数字且最多保留两位小数`));
    }
    if (_value > 100) {
  ...
  donationLimitRate: (rule, value, callback) => {
    ...
    if (!reg.test(_value)) {
      return callback(new Error(rule.message || `${rule.title}必须为数字且最多保留两位小数`));
    }
    if (_value > 100) {
      return callback(new Error(`${rule.title}不能超过100`));
    }
